...
if __name__ == '__main__':
    ...
    fwdPS_ping = net_connect.send_command('ping 10.124.153.90' + " " + 'rep 3\n')
    ...
    fwdPS_port = net_connect.send_command('sh run int Gi2/0/41')
    ...
    fwd_mac = net_connect.send_command('sh mac add int Gi2/0/41')
    mac_add = input("==> do you want to see mac-add learned by Gi2/0/41?, (Y) to continue ("
                    "N) to cancel:").lower()
    if mac_add in yes_option:
        print('=> Getting mac-add learned on Gi2/0/41')
        print(fwd_mac)
    else:
        pass
